...
def check_annotations(file_name, img_dir, ann_dir, show=True, output_dir=None):
    img = img_dir + file_name + '.JPG'
    ...
    with open(ann_file, 'r') as ann:
        ...
        for line in lines:
            ...
            try:
                x1, y1, x2, y2, x3, y3, x4, y4, classname, diffculty = arr
            except ValueError:
    ...
    for item in ann_list:
        ...
        img = cv2.putText(img, classname, (x1-5, y1-5), cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255,0,0), thickness=5)
...
if __name__ == '__main__':
    img_dir = 'Train/'
    ann_dir = 'DOTA/labelTxt/'
    filename = '2012-04-26-Muenchen-Tunnel_4K0G0020.txt'
    filename = filename.replace('.txt', '')
    check_annotations(file_name=filename, img_dir=img_dir, ann_dir=ann_dir, show=False, output_dir = 'output/')
